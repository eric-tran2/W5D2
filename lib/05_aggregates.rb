...
def area_count
  ...
  execute(<<-SQL)
    SELECT COUNT(area) AS big_boy_area
...
def country_counts
  ...
  execute(<<-SQL)
    SELECT continent, COUNT(continent) AS country_count
...
def populous_country_counts
  ...
  execute(<<-SQL)
    SELECT continent, COUNT(continent) AS pop_over_10M
    FROM countries WHERE population >= 10000000 GROUP BY continent;
  SQL
end
