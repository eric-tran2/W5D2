...
def per_capita_gdp
  ...
  execute(<<-SQL)
    SELECT name, (gdp / population) AS gdp_per_capital 
    FROM countries WHERE area > 5000000 ORDER BY area desc limit 6;
  SQL
end
...
def small_and_wealthy
  ...
  execute(<<-SQL)
    ...
    FROM countries WHERE area < 2000 AND gdp > 5000000000;
  SQL
end
...
def scandinavia
  ...
  execute(<<-SQL)
    ...
    FROM countries WHERE  name in ('Denmark','Finland','Norway','Sweden');
  SQL
end
...
def starts_with_g
  ...
  execute(<<-SQL)
    SELECT name
    FROM countries WHERE name LIKE 'G%';
  SQL
end
