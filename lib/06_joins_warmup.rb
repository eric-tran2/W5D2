...
def year_of_kane
  ...
  execute(<<-SQL)
    SELECT yr
    FROM movies WHERE title = 'Citizen Kane';
  SQL
end
...
def trek_films
  ...
  execute(<<-SQL)
    SELECT id, title, yr
    FROM movies WHERE title LIKE 'Star Trek%' ORDER BY yr;
  SQL
end
...
def films_by_id
  ...
  execute(<<-SQL)
    SELECT title
    FROM movies WHERE id IN(1119, 1595, 1768);
  SQL
end
...
def casablanca_id
  ...
  execute(<<-SQL)
    SELECT id
    FROM movies WHERE title = 'Casablanca';
  SQL
end
...
def casablanca_cast
  ...
  execute(<<-SQL)
    SELECT actors.name, castings.movie_id
    FROM actors JOIN castings ON castings.actor_id = actors.id
